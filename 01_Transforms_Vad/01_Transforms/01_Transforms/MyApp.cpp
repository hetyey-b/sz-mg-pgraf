...
/* float cam_look_at_y = 0; */
/* float cam_look_at_y_speed = 0.01; */
void CMyApp::Update()
{
    /* cam_look_at_y += cam_look_at_y_speed; */
    /* if (cam_look_at_y > 1 || cam_look_at_y < -1) { */
    /*     cam_look_at_y_speed *= -1; */
    /* } */
...
void CMyApp::Render()
{
	...
	*/
    ...
    // forduljon meg 3mp-ként
    float rotateSpeed = 0.33f;
    float rotateAngle = timeElapsedInSec * 360.0f * rotateSpeed;
    if (rotateAngle >= 360.0f) {
        rotateAngle -= 360.0f;
    }

    /* m_matWorld = glm::rotate(glm::radians(rotateAngle), glm::vec3(1.0,0.0,0.0)) * */
    /*                 glm::scale(glm::vec3(2.0f, 2.0f, 1.0f)); */
    // fordított sorrend: Először skáláz, aztán forgat
    
    // skálázás forgatás eltolás, ill. skálázás eltolás forgatás
    m_matWorld = glm::rotate(glm::radians(rotateAngle), glm::vec3(0.0,1.0,0.0)) *
                    glm::translate(glm::mat4(1.0f), glm::vec3(1.0,0.0,0.0)) *
                    glm::scale(glm::vec3(1.5f, 1.5f, 1.0f));

    /* m_matWorld = glm::translate(glm::mat4(1.0f), glm::vec3(1.0,0.0,0.0)) * */
    /*                 glm::rotate(glm::radians(rotateAngle), glm::vec3(0.0,1.0,0.0)) * */
    /*                 glm::scale(glm::vec3(1.5f, 1.5f, 1.0f)); */
    ...
    glUniform3f(glGetUniformLocation(m_programID, "color"),1.0,0.33,1.0);
