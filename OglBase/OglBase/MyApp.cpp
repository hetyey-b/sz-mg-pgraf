...
void CMyApp::Update()
{
	...
	static float last_time_in_secs = last_time / 1000.0f;

	float delta_time = (SDL_GetTicks() - last_time) / 1000.0f;
    if (m_scene_IsAnimOn) {
        last_time_in_secs += delta_time;
    }
	...
	float rotation_angle = last_time_in_secs * 2.0f * float(M_PI) * m_scene_CubeTurnsPerSec;

	// Update the positions and orientations of the cubes
	for (int i = 0; i < 10; ++i)
	{
		glm::mat4& cubeWorld = m_cubeWorldArray[i];
		cubeWorld =
			glm::rotate(rotation_angle + 2.0f * glm::pi<float>() / 10 * i, glm::vec3(0, 1, 0)) *
			glm::translate(glm::vec3(10 + 5 * sin(rotation_angle), 0, 0)) *
			glm::rotate((i + 1) * rotation_angle, glm::vec3(0, 1, 0));
	}

    m_cursorColor = glm::vec3(fabsf(sinf(SDL_GetTicks()/1000.0f*2.0f*M_PI)));
}
...
void CMyApp::Render()
{
    ...
    // global
    m_program.SetUniform("light_pos", m_lightPosition);
	...
	for (int i = 0; i < 10; ++i)
	{
		const glm::mat4& cubeWorld = m_cubeWorldArray[i];
        ...
        if (m_scene_selectedCubeIdx == i) {
            m_program.SetUniform("Remmisive ", m_cursorColor);
        } else {
            m_program.SetUniform("Remmisive ", glm::vec3(0.0f));
        }
    ...
    if (ImGui::Begin("Gyak10")) {
        ImGui::SliderFloat3("Light dir", &m_lightPosition[0], -1.0f, 1.0f);
        ImGui::ColorEdit3("Light color", &m_Ld[0],ImGuiColorEditFlags_Float);
        ImGui::Separator();
        ImGui::SliderFloat("Turns/s", &m_scene_CubeTurnsPerSec, 0.0f, 2.0f);
        ImGui::Separator();
        ImGui::LabelText("Elapsed Time", "%f", SDL_GetTicks() / 1000.0f);
        ImGui::Separator();
        static bool IsAnimSettable = true;
        ImGui::Checkbox("Anim settable", &IsAnimSettable);
        if (IsAnimSettable) {
            if (ImGui::Button((m_scene_IsAnimOn?"Pause":"Play"))) {
                m_scene_IsAnimOn = !m_scene_IsAnimOn;
            }
        }

        static int cubenum = 10;
        ImGui::SliderInt("Num of cubes0", &cubenum, 0, 15);
        ImGui::Separator();

        ImGui::ListBoxHeader("Cube list", (int)m_cubeWorldArray.size());
        for (int i = 0; i < m_cubeWorldArray.size(); i++) {
            if (ImGui::Selectable(std::to_string(i).c_str(),i == m_scene_selectedCubeIdx)) {
                m_scene_selectedCubeIdx = i;
            }
        }
        ImGui::ListBoxFooter();
    }
