...
{
	m_tableTextureID = TextureFromFile("./assets/wood.jpg");
	...
	m_sphereTextureID = TextureFromFile("./assets/marron.jpg");
	// mintavételezés beállításai
...
bool CMyApp::Init()
{
    ...
    m_loc_light_dir = glGetUniformLocation(m_programID, "lightDir");
    m_loc_light_switch = glGetUniformLocation(m_programID, "light_switch");
    m_loc_light_battery = glGetUniformLocation(m_programID, "light_battery");
...
void CMyApp::Update()
{
    ...
    if (swapping_battery) {
        m_scene_isLampOn = false;
        battery_swap_progress += delta_time;
        if (battery_swap_progress >= 2.0f) {
            battery_swap_progress = 0.0f;
            swapping_battery = false;
            m_lightBattery = 100.0f;
        }
    }

    if (m_lightBattery > 0.0f && m_scene_isLampOn) {
        m_lightBattery = std::max(0.0f, m_lightBattery - (delta_time*10.0f));
    }
...
void CMyApp::Render()
{
	...
	*/
	...
	glUniform3fv(m_loc_light_dir, 1, &m_lightDir[0]);
	...
	glUniform1f(m_loc_light_switch, m_scene_isLampOn);
	glUniform1f(m_loc_light_battery, m_lightBattery);
	// asztal rajzolasa
    ...
    if (ImGui::Begin("Beadando 2")) {
        if (m_scene_isLampOn && m_lightBattery > 0.0f && !swapping_battery) {
            ImGui::Text("Light is on");
        } else {
            ImGui::Text("Light is off");
        }
        ImGui::Separator();
        ImGui::SliderFloat("Battery", &m_lightBattery, 0.0f, 100.0f, "%.1f%%");
        ImGui::Separator();
        if (ImGui::Button((swapping_battery?"Swapping...":"Swap Battery"))) {
            swapping_battery = true;
        }
    }
